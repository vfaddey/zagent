...
- внутрь контейнера монтируется папка `.agent`;
- внутренний runtime считывает `run.yaml` и `.agent`;
...
### 4.2. Внутреннее окружение `.agent`
...
mcp:
  servers_file: mcp/servers.yaml

files:
  extra_context:
    - files/extra_context.md
```
...
- `.agent` — runtime-окружение агента.

Это позволяет:

- переиспользовать `.agent` между разными запусками;
...
Runtime и launcher не должны импортировать код друг друга. Их связь идет через стабильные внешние контракты: `run.yaml`, `.agent/config.yaml`, runtime artifacts и JSON Schema.
...
После загрузки `run.yaml` и `.agent` должен собираться единый `RuntimeContext`.
...
Финальное сообщение должно нормализоваться runtime-слоем: если agent не вернул configured `runtime.final_marker`, executor добавляет его сам. Это дает стабильный внешний сигнал завершения run даже при неполном ответе модели.
...
Для задач исправления кода git должен быть центральной сущностью runtime.
...
- один AG2 agent;
- built-in tools: `shell`, `files`, `git`, `apply_patch`;
- MCP support через registry;
- `.agent` как базовое runtime-окружение;
...
contracts/agent-env.schema.json
...
- `.agent` является стабильным внутренним окружением агента;
